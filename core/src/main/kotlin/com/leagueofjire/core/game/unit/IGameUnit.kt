...
import com.leagueofjire.game.unit.GameMissile
...
open class IGameUnit : GameUnit, GameChampion, GameMinion, GameMissile {
	...
	override var missileName = ""
	override var missileStartPos: GamePosition = IGamePosition()
	override var missileEndPos: GamePosition = IGamePosition()
	override var missileSource = -1
	override var missileDestiny = -1
	...
	private fun updateMissiles(process: AttachedProcess, data: Pointer): Boolean {
		try {
//			val mStartPos = IGamePosition()
//			val mEndPos = IGamePosition()
//			val spellInfoPtr = data.getInt(Offsets.MissileSpellInfo)
//			if (spellInfoPtr <= 0){
//				return false
//			}
//			val spellDataPtr = process.int(spellInfoPtr.toLong() + Offsets.SpellInfoSpellData)
//			if (spellDataPtr <= 0){
//				return false
//			}
//
//			val buffPointerAlloc = Pointer.alloc(0x500)
//			if (!process.read(spellDataPtr.toLong(), buffPointerAlloc, 0x500)) {
//				return false
//			}
//			val spellNamePtr = buffPointerAlloc.getInt(Offsets.SpellDataMissileName) // hasta aca bien.
//
//			val mName = RiotStrings().riotString(process, spellNamePtr.toLong())
//			if	(mName.isEmpty()) return true
//			val mSrcIdx = data.getShort(Offsets.MissileSrcIdx)
//
//			val mDestIdx = data.getShort(Offsets.MissileDestIdx)
//
//			mStartPos.x = data.getFloat(Offsets.MissileStartPos)
//			mStartPos.y = data.getFloat(Offsets.MissileStartPos + 4)
//			mStartPos.z = data.getFloat(Offsets.MissileStartPos + 8)
//
//			mEndPos.x = data.getFloat(Offsets.MissileEndPos)
//			mEndPos.y = data.getFloat(Offsets.MissileEndPos + 4)
//			mEndPos.z = data.getFloat(Offsets.MissileEndPos + 8)
//
//			missileName = mName
//			missileSource = mSrcIdx.toInt()
//			missileDestiny = mDestIdx.toInt()
//			missileStartPos = mStartPos
//			missileEndPos = mEndPos
//			for (spell in spells){
//				println(spells)
//				// Find static data
//				if (spell.name == missileName){
//					// Some spells require their end position to be projected using the range of the spell
//					if (spell.info != SpellInfo.unknownSpell && spell.info.projectDestination){
//						missileStartPos.y += spell.info.height
//						// Calculate direction vector and normalize
//						missileEndPos = subGamePosition(missileEndPos, missileStartPos)
//						missileEndPos = normalize(missileEndPos)
//						// Update endposition using the height of the current position
//						missileEndPos.x = missileEndPos.x * spell.info.castRange + SmissileStartPos.x
//						missileEndPos.y = missileStartPos.y
//						missileEndPos.z = missileEndPos.z * spell.info.castRange + missileStartPos.z
//					}
//				}
//			}
//			missiles.add(IGameMissile(
//				missileSrcIdx
//				,missileDestIdx
//				,missileStartPos
//				,missileEndPos
//				,missileName))
			...
			println("Error in IGameUnit.updateMissiles: " + ex.message)
			return true
		}
	}
	...
	private fun updateBuff(process: AttachedProcess, data: Pointer): Boolean {
		try {
			...
			while (currentAddress != buffArrayEnd){
				...
				val nameBuff = RiotStrings().riotString(process, dataBuff.toLong() + Offsets.BuffName)
				if (nameBuff.isEmpty()) {
					currentAddress += ADDRESS_BUFF_ITERATOR
					continue
				}
				val countAlt = (buffPointerAlloc.getInt(Offsets.BuffEntryBuffCountAlt) - buffPointerAlloc.getInt(Offsets.BuffEntryBuffCountAlt2)) shr 3

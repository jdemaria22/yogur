...
class ITargeting : Targeting {
    ...
    override fun getBestTargetForRangeMonster(monster: ObjectList<IGameUnit>, localPlayer: IGameUnit, range: Float): Optional<IGameUnit> {
        var lastHealth = Float.MAX_VALUE
        var lastDistance = Float.MAX_VALUE
        var target: Optional<IGameUnit> = Optional.empty()
        monster
    ...
    override fun getBestTargetForRangeMinion(minions: ObjectList<IGameUnit>, localPlayer: IGameUnit, range: Float): Optional<IGameUnit> {
        var lastHealth = Float.MAX_VALUE
        var lastDistance = Float.MAX_VALUE
        var target: Optional<IGameUnit> = Optional.empty()
        minions
    ...
    companion object {
        ...
        const val PASSIVE_GRAVES_GRIT = "gravesegrit"
    }
    ...
    fun isGravesAmmoActive(buffs: ArrayList<GameChampionBuff>, time: GameTime): Boolean{
        if (buffs.isEmpty()) return false
        val buffFiltered = buffs.filter {
            (it.endTime > time.seconds) &&
            (it.name == PASSIVE_GRAVES_1 && it.countAlt > 0)
                    ||
            (it.name == PASSIVE_GRAVES_2 && it.countAlt > 0)
        }
        if (buffFiltered.isEmpty()) return false
        return true
    }
